...
export const HOLDOUT_V6_SPLIT: GroundedEvaluationSplit = "holdout_v6";
export const HOLDOUT_V6_FIXTURE_SCHEMA_VERSION =
  "grounded-holdout-v6-fixture-v1";
export const HOLDOUT_V6_CREATED_AT = "2026-08-16T00:00:00.000Z";
export const HOLDOUT_V6_SOURCE_HEAD =
  "1bc413366c61d0cf763e1e1e9e658dad3f51a11a";
export const HOLDOUT_V6_RUN_RECORD_SCHEMA_VERSION =
  "grounded-holdout-v6-run-record-v1";
export const HOLDOUT_V6_RUN_RECORD_PREFIX = "holdout-v6-acceptance";

export const HOLDOUT_V6_BEHAVIOR_FILE_PATHS = [
  ...
  "lib/ai/grounding/classification.ts",
  "lib/ai/grounding/config.ts",
  "lib/ai/grounding/evidence.ts",
  "lib/ai/grounding/grounded-generation-service.ts",
  "lib/ai/grounding/grounding-validator.ts",
  "lib/ai/grounding/prompt.ts",
  "lib/ai/grounding/query-builder.ts",
  "lib/ai/grounding/structured-output.ts",
  "lib/ai/grounding/sufficiency.ts",
...
const HOLDOUT_V6_PROMPT_VERSION = "grounded-teach-prompt-v1.6";
const HOLDOUT_V6_GROUNDING_VERSION = "stage4-grounded-teach-v1";
const HOLDOUT_V6_SUFFICIENCY_POLICY_VERSION = "sufficiency-policy-v1.15";
const HOLDOUT_V6_VALIDATOR_VERSION = "grounding-validator-v1.11";

export const HOLDOUT_V6_FROZEN_CONFIG = {
  prompt: HOLDOUT_V6_PROMPT_VERSION,
  grounding: HOLDOUT_V6_GROUNDING_VERSION,
  sufficiency: HOLDOUT_V6_SUFFICIENCY_POLICY_VERSION,
  validator: HOLDOUT_V6_VALIDATOR_VERSION,
  chatProvider: "openai",
  ...
  conceptCompatibilityConfiguration: {
    version: HOLDOUT_V6_SUFFICIENCY_POLICY_VERSION,
...
export const HOLDOUT_V6_ACCEPTANCE_GATES = {
  safety: {
  ...
  recommendationRules: {
    holdoutFailure: "DO_NOT_ENABLE",
    allSyntheticGatesPass: "ENABLE_FOR_INTERNAL_TEST_USERS_MAX",
    productionFromSingleHoldout: "PROHIBITED",
    historicalV5FailureRemainsDisclosed: true,
  },
} as const;

export interface HoldoutV6SplitSummary {
  splitHash: string;
  caseCount: number;
...
export interface HoldoutV6GuardInput {
...
export interface HoldoutV6ContaminationFinding {
  type:
    | "DUPLICATE_ID"
...
export function summarizeHoldoutV6Split(input: {
  cases: GroundedEvaluationCase[];
  resources: GroundedEvaluationResource[];
}) {
  const holdoutCases = selectHoldoutV6Cases(input.cases);
  return {
    splitHash: computeHoldoutV6SplitHash(input),
    ...
    resourceCount: collectHoldoutV6ResourceIds(holdoutCases, input.resources).size,
    fixtureSchemaVersion: HOLDOUT_V6_FIXTURE_SCHEMA_VERSION,
    createdAt: HOLDOUT_V6_CREATED_AT,
    sourceHead: HOLDOUT_V6_SOURCE_HEAD,
    caseIds: holdoutCases.map((item) => item.id),
  } satisfies HoldoutV6SplitSummary;
}

export function computeHoldoutV6SplitHash(input: {
  cases: GroundedEvaluationCase[];
  resources: GroundedEvaluationResource[];
}) {
  const splitCases = selectHoldoutV6Cases(input.cases).map(canonicalCase);
  const resourceIds = collectHoldoutV6ResourceIds(
    selectHoldoutV6Cases(input.cases),
    input.resources
  );
  ...
  return hashJson({
    schemaVersion: HOLDOUT_V6_FIXTURE_SCHEMA_VERSION,
...
export function validateHoldoutV6FixtureReferences(input: {
  cases: GroundedEvaluationCase[];
  resources: GroundedEvaluationResource[];
}) {
  ...
  for (const item of selectHoldoutV6Cases(input.cases)) {
    ...
    for (const resourceId of duplicates(item.corpusResourceIds ?? [])) {
...
export function analyzeHoldoutV6Contamination(input: {
  cases: GroundedEvaluationCase[];
  resources: GroundedEvaluationResource[];
  testCorpus?: string;
}) {
  const holdoutCases = selectHoldoutV6Cases(input.cases);
  const otherCases = input.cases.filter((item) => item.split !== HOLDOUT_V6_SPLIT);
  const holdoutResourceIds = collectHoldoutV6ResourceIds(holdoutCases, input.resources);
  ...
  const findings: HoldoutV6ContaminationFinding[] = [];
...
export async function assertHoldoutV6AcceptanceRunAllowed(
  input: HoldoutV6GuardInput
) {
  if (input.confirmSplitHash !== input.computedSplitHash) {
    throw new Error("holdout_v6 requires explicit matching split hash confirmation.");
  }

  if (!input.allowDiagnostic && (input.caseIds?.length || input.maxCases)) {
    throw new Error(
      "holdout_v6 acceptance must run the complete split. Use diagnostic mode for partial checks."
    );
  }

  if (input.allowDiagnostic) return;

  const recordPath = holdoutV6RunRecordPath({
  ...
  try {
    ...
    throw new Error(
      "holdout_v6 already has an acceptance-run record. Use diagnostic mode only."
    );
  } catch (error) {
...
export async function recordHoldoutV6AcceptanceRun(
  input: HoldoutV6RunRecordInput
) {
  return writeHoldoutV6AcceptanceRun(input, "wx");
}

export async function updateHoldoutV6AcceptanceRun(
  input: HoldoutV6RunRecordInput
) {
  return writeHoldoutV6AcceptanceRun(input, "w");
}

export function holdoutV6RunRecordPath(input: {
  splitHash: string;
  reportDir?: string;
}) {
  ...
  return path.join(
    reportDir,
    `${HOLDOUT_V6_RUN_RECORD_PREFIX}-${input.splitHash}.json`
  );
}

async function writeHoldoutV6AcceptanceRun(
  input: HoldoutV6RunRecordInput,
  flag: "w" | "wx"
) {
  const reportDir = path.resolve(
  ...
  const record = {
    schemaVersion: HOLDOUT_V6_RUN_RECORD_SCHEMA_VERSION,
    split: HOLDOUT_V6_SPLIT,
    fixtureHash: input.fixtureHash,
    splitHash: input.splitHash,
    candidateHead: input.candidateHead,
    candidateDiffHash: input.candidateDiffHash,
    candidateTreeHash: input.candidateTreeHash ?? null,
    candidateBehaviorHash: input.candidateBehaviorHash ?? null,
    behaviorFilePaths: input.behaviorFilePaths ?? HOLDOUT_V6_BEHAVIOR_FILE_PATHS,
    frozenConfig: HOLDOUT_V6_FROZEN_CONFIG,
    runId: input.runId,
...
function selectHoldoutV6Cases(cases: GroundedEvaluationCase[]) {
  return cases.filter((item) => item.split === HOLDOUT_V6_SPLIT);
}

function collectHoldoutV6ResourceIds(
  cases: GroundedEvaluationCase[],
  resources: GroundedEvaluationResource[]
) {
