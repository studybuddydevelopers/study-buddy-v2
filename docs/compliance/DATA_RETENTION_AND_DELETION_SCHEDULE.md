...
| Exceptional age/guardian verification evidence | Only while a disputed, inconsistent, or suspicious case is verified | Delete the raw evidence immediately after recording the verification result unless a documented legal requirement justifies limited retention | Identity documents are not collected routinely; retain only a minimised result and evidence reference |
...
| Password-reset abuse and temporary-lock state | While needed to deliver the warning, enforce the lock and complete recovery; audit events follow the security-event period | Raw email and bearer tokens are not stored in the application security tables; one-way token hashes expire after 30 minutes or 24 hours as applicable and are cleared after recovery; delete account-linked state with the account | Alert/lock/recovery controls are implemented; production integration testing and the central security-event retention job remain pending |
