...
| ROPA-08 | Manage automatically renewing subscriptions and payment verification | Subscribers/payers; user ID while the account exists, plan, recurring-billing authorisation, cancellation request/confirmation, reference, amount, currency, status and dates; no full card/bank credentials stored | Contract; legal obligation; fraud-prevention legitimate interests | Paystack, Supabase, hosting, financial/regulatory recipients where legally required | Essential payment, invoice, and transaction records: seven years from transaction date; longer only for a documented audit, chargeback, dispute, investigation, or legal hold | Automatic renewal and Settings → Subscription cancellation are approved but not enabled. Cancellation stops renewal, sends confirmation, and ends access on the cancellation date; billing email is the fallback. Unused prepaid-time treatment, implementation, expiry/legal-hold automation, and accountant/legal review remain pending |
