---
name: design-taste-frontend-v1
description: Bản legacy ổn định hơn, ít phá cách hơn, dùng khi v2 quá mạnh hoặc cần sửa nhỏ.
---

# Taste Skill VI v1 - Legacy

Dùng khi dự án cần cải thiện UI an toàn, không muốn thay đổi mạnh.

## Nguyên tắc

- Giữ cấu trúc cũ nếu người dùng không yêu cầu overhaul.
- Sửa spacing, typography, hierarchy, color trước khi thêm animation.
- Không đổi framework nếu không cần.
- Không thêm dependency lớn để chỉ làm đẹp.

## Preset an toàn

```text
DESIGN_VARIANCE: 4-6
MOTION_INTENSITY: 1-3
VISUAL_DENSITY: giữ theo dự án hiện tại
```

## Checklist

- Heading rõ hơn
- Khoảng cách đều hơn
- Button có hierarchy chính/phụ
- Card bớt giống nhau
- Mobile không vỡ
- Màu thương hiệu rõ hơn
- Empty/loading/error có nội dung dễ hiểu

## Quy trình bắt buộc

1. Đọc brief và inventory hiện có.
2. Xác định người dùng thật, không thiết kế theo gu mơ hồ.
3. Chọn cảm giác thương hiệu trước khi chọn màu/component.
4. Đặt ba biến: `DESIGN_VARIANCE`, `MOTION_INTENSITY`, `VISUAL_DENSITY`.
5. Chỉ dùng hiệu ứng nếu nó giúp người dùng hiểu hoặc thao tác tốt hơn.
6. Thiết kế mobile-first, sau đó mới mở rộng desktop.
7. Trả về PASS/FAIL rõ ràng trước khi code.

## Anti-slop mặc định

Không mặc định dùng:

- gradient tím/xanh kiểu AI SaaS
- hero căn giữa + subtitle + 2 CTA + mockup lơ lửng
- 3 feature card ngang giống nhau
- glassmorphism toàn bộ giao diện
- font Inter/slate mặc định khi không có lý do
- icon/emoji lộn xộn
- animation vô hạn gây phân tâm

## Output trước khi làm

```text
DESIGN_READ: ...
DIALS: variance/motion/density = ...
PLAN: ...
ANTI_SLOP_RISKS: ...
VERDICT: PASS_TO_CODE hoặc NEEDS_BRIEF
```
