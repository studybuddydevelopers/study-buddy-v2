---
name: imagegen-frontend-mobile
description: Tạo prompt ảnh tham chiếu mobile/app screen có hierarchy rõ và dùng được.
---

# ImageGen Frontend Mobile VI

Dùng để tạo màn hình mobile/app trước khi code.

## Ưu tiên

- thao tác một tay
- vùng chạm đủ lớn
- text ngắn, rõ
- bottom nav/primary action hợp ngữ cảnh
- không nhồi quá nhiều card
- trạng thái empty/loading/error

## Prompt mẫu

```text
Create a high-quality mobile app screen for [task], used by [audience].
Design should feel [vibe], with clear hierarchy, large touch targets, Vietnamese-friendly text length, and practical production UI.
Avoid generic neumorphism, random gradients, tiny text, and decorative motion.
```

## Quy trình bắt buộc

1. Đọc brief và inventory hiện có.
2. Xác định người dùng thật, không thiết kế theo gu mơ hồ.
3. Chọn cảm giác thương hiệu trước khi chọn màu/component.
4. Đặt ba biến: `DESIGN_VARIANCE`, `MOTION_INTENSITY`, `VISUAL_DENSITY`.
5. Chỉ dùng hiệu ứng nếu nó giúp người dùng hiểu hoặc thao tác tốt hơn.
6. Thiết kế mobile-first, sau đó mới mở rộng desktop.
7. Trả về PASS/FAIL rõ ràng trước khi code.

## Anti-slop mặc định

Không mặc định dùng:

- gradient tím/xanh kiểu AI SaaS
- hero căn giữa + subtitle + 2 CTA + mockup lơ lửng
- 3 feature card ngang giống nhau
- glassmorphism toàn bộ giao diện
- font Inter/slate mặc định khi không có lý do
- icon/emoji lộn xộn
- animation vô hạn gây phân tâm

## Output trước khi làm

```text
DESIGN_READ: ...
DIALS: variance/motion/density = ...
PLAN: ...
ANTI_SLOP_RISKS: ...
VERDICT: PASS_TO_CODE hoặc NEEDS_BRIEF
```
